...
\begin{document}
	...
	%=====================================================================================
	\section{演習問題(斜方投射)}
	問1.小球を水平な地面から仰角$30°$、速さ$39.2\,\si{\metre\per\second}$で投げた。
	重力加速度の大きさを$9.8\,\si{\metre\per\second\squared}$,水平方向を　　\,\,$x$軸の正、
	鉛直上向きを$y$軸の正として以下の問いに答えよ。\\
	(1)初速度の$x$成分及び$y$成分は何$\si{\metre\per\second}$か。\\\\\\\\\\\\
	(2)最高点に達するまでの時間は何$\si{\second}$か。\\\\\\\\\\\\
	(3)最高点の高さは何$\si{\metre}$か。\\\\\\\\\\\\
	(4)投げてから地面に達するまでの時間は何$\si{\second}$か。\\\\\\\\\\\\\\
	(5)小球が地面に落ちたときの水平距離は何$\si{\metre}$か。
	\clearpage
\end{document}